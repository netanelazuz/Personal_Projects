...
function Remove-UserProfile($username) {
    $userPath = Join-Path -Path "\\$computername\C$" -ChildPath "Users\$username"
...
function Remove-UserFromRegistry($username) {
    ...
    $registryPath = "\\$computername\HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList\$sid"
...
function Operation($selectedUsers) {
     ...
     foreach ($selectedUser in $selectedUsers) {
        ...
        $sessionInfo = psexec \\$ComputerName quser | Where-Object { $_ -match $selectedUser }
...
# Function to create a pop-up window for entering computer name
function Show-ComputerNamePopup {
    Add-Type -AssemblyName System.Windows.Forms

    # Create the pop-up form
    $popupForm = New-Object System.Windows.Forms.Form
    $popupForm.Text = "Enter Computer Name"
    $popupForm.Size = New-Object System.Drawing.Size(400, 150)
    $popupForm.StartPosition = "CenterScreen"

    # Label for computer name
    $label = New-Object System.Windows.Forms.Label
    $label.Text = "Insert Computer Name:"
    $label.AutoSize = $true
    $label.Location = New-Object System.Drawing.Point(20, 20)
    $popupForm.Controls.Add($label)

    # Text box for entering computer name
    $textBox = New-Object System.Windows.Forms.TextBox
    $textBox.Location = New-Object System.Drawing.Point(($label.Location.X + $label.Width + 10), $label.Location.Y)
    $textBox.Size = New-Object System.Drawing.Size(200, 20)
    $popupForm.Controls.Add($textBox)

    # Button to run on local computer
    $localButton = New-Object System.Windows.Forms.Button
    $localButton.Text = "Run on Local Computer"
    $localButton.Location = New-Object System.Drawing.Point(20, 80)
    $localButton.Size = New-Object System.Drawing.Size(150, 30)
    $localButton.Add_Click({
        $Global:ComputerName = hostname
        $popupForm.DialogResult = [System.Windows.Forms.DialogResult]::OK
    })
    $popupForm.Controls.Add($localButton)

    # OK button to save computer name
    $okButton = New-Object System.Windows.Forms.Button
    $okButton.Text = "OK"
    $okButton.Location = New-Object System.Drawing.Point(($popupForm.Width - 100 - 20), 80)
    $okButton.Size = New-Object System.Drawing.Size(100, 30)
    $okButton.Add_Click({
        $Global:ComputerName = $textBox.Text
        $popupForm.DialogResult = [System.Windows.Forms.DialogResult]::OK
    })
    $popupForm.Controls.Add($okButton)

    # Show the pop-up window
    $popupResult = $popupForm.ShowDialog()

    if ($popupResult -eq [System.Windows.Forms.DialogResult]::OK) {
...
function Main() {
    ...
    #Name of the computer the script will execute on.
    $Global:ComputerName = ""

    # Call the function to show the pop-up window
    Show-ComputerNamePopup

    # Get the current username
    $Global:currentUsername = (Get-WmiObject -Class Win32_ComputerSystem | Select-Object -ExpandProperty UserName).Split("\")[1]

    # Get list of user profiles on the local machine (excluding system profiles)    
    $Global:usernames = Get-ChildItem -Path "\\$computername\C$\Users" | Where-Object { $_.PSIsContainer -and $_.Name -notin @("Administrator", "Default", "Public", "$currentUsername", "All Users") } | Select-Object -ExpandProperty Name

    # Initialize $selectedUsers
    $Global:selectedUsers = $null
