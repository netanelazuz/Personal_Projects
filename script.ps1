...
function Show-ComputerNamePopup {
    ...
    if ($popupResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        $popupForm.Dispose()
        Gui # Call the main GUI function after the pop-up window is closed
    } else {
        $popupForm.Dispose()
    }
}
...
$usernames = Get-ChildItem -Path "\\$computername\C$\Users" | Where-Object { $_.PSIsContainer -and $_.Name -notin @("Administrator", "Default", "Public", "$currentUsername", "All Users") } | Select-Object -ExpandProperty Name
